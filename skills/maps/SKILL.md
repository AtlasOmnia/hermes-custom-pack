---
name: maps
description: maps — Geocode, POIs, routes, timezones via OpenStreetMap/OSRM.
version: 1.2.0
author: Mibayy
license: MIT
platforms:
...
1. Use airline and airport sources as authoritative for route availability, schedules, fares/points, terminal info, and recommended arrival times; map route lookup is supplementary.
2. Use `search` to verify airport coordinates / exact airport identity when names are ambiguous.
3. Use `timezone` when the itinerary crosses timezones or the user needs local arrival/departure context.
4. Use `distance` or `directions` only as supplementary ground-planning tools (airport to hotel, airport to family, hotel to attraction) — not as the source of truth for flight availability.
