...
| [hermes-diagnostic-review](skills/hermes-diagnostic-review/) | Use when running a read-only diagnostic review of recent Hermes sessions to find recurring mistakes, failed tool calls, and repeated fixes, then propose suggestion-only improvements and reusable skills. Human-gated; never auto-applies | Ready |
...
| [reddit-browse-and-post](skills/reddit-browse-and-post/) | Read-only Reddit browsing, search, and thread reading by default; posting is a separately approved opt-in through the user's authenticated session or OAuth app. Account-agnostic; no credentials in the skill | Ready |
